          ...
          <div className="px-6 py-2 bg-slate-100 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-500 font-bold uppercase">Estado Supabase</span>
                <span className={`text-xs font-black ${supabaseLog?.status.includes('Error') ? 'text-red-500' : 'text-emerald-600'}`}>
                  {supabaseLog?.status || 'Conectando...'}
                </span>
              </div>
              <div className="h-6 w-px bg-slate-300" />
              <div className="flex gap-4">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Salas</span>
                  <span className="text-xs font-black text-slate-700">{supabaseLog?.rooms || 0}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-500 font-bold uppercase">Charlas</span>
                  <span className="text-xs font-black text-slate-700">{supabaseLog?.events || 0}</span>
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              <button 
                ...
                className="p-1.5 hover:bg-red-50 rounded-lg text-red-500 transition-colors flex items-center gap-2 border border-transparent hover:border-red-200"
                title="Limpiar caché local y re-sincronizar"
              >
                <Trash2 className="w-4 h-4" />
                <span className="text-[10px] font-bold uppercase">Limpiar Caché</span>
              </button>
              <button 
                onClick={syncData}
                className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-600 transition-colors flex items-center gap-2"
                title="Sincronizar ahora"
              >
                <Download className="w-4 h-4" />
                <span className="text-[10px] font-bold uppercase">Forzar Sincronización</span>
              </button>
            </div>
          </div>

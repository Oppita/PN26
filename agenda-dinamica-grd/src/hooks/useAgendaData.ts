...
export function useAgendaData() {
      ...
      try {
        ...
        if (user) {
          const { data: regData, error: regError } = await supabase
            ...
            .eq('user_id', user.id);
          if (!regError && regData) {
            setRegistrations(new Set<string>(regData.map((r: any) => r.talk_id)));
          }
        }
        ...
        if (!talksError && talksData && talksData.length > 0) {
          const mappedTalks: AgendaEvent[] = talksData.map((t: any) => ({
            ...
            speakers: typeof t.speakers === 'string' ? JSON.parse(t.speakers) : t.speakers || [],
  ...
  const registerForEvent = async (eventId: string) => {
    ...
    try {
      const supabase = getSupabase();
      await supabase.from('registrations').insert({ user_id: user.id, talk_id: eventId });
  ...
  const cancelRegistration = async (eventId: string) => {
    ...
    try {
      const supabase = getSupabase();
      await supabase.from('registrations').delete().match({ user_id: user.id, talk_id: eventId });
